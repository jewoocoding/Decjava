package com.dec.day04.typetrans;
...
public class Exam_TypeTrans {
	public static void main(String[] args) {
		 ...
		 * 강제 형변환과 자동 형변환을 사용
		 * (double)36/5 = 7.2 -> / 앞은 강제 형변환, /뒤는 자동 형변환
		...
		// 자동형변환의 예
		// - iNum이 정수에서 실수로 형변환
		double result = dNum * iNum; // iNum이 자동형변환이 된다.
		System.out.println(result);
		
		// 강제형변환의 예
		// 1. 큰 자료형을 작은 자료형으로 변환
		System.out.println("dNum : "+(int)dNum); // 소수점이 잘림
		// 2. 작은 자료형을 큰 자료형으로 변환
		// - 평균 구할 때 사용했었음.
		result = (double)iNum / iNum;
		System.out.println("result : "+result);
		// 강제 형변환의 예2
		System.out.println((char)65); // 정수(int)값을 문자(char)값으로 강제 형변환
